...
import { estimateDayMinutes, totalSets } from '../lib/duration'
import { dayEquipment, equipmentBadge, programEquipment } from '../lib/equipment'
    ...
    <div className="mx-auto max-w-lg px-4 pb-24 pt-6">
      ...
      <span
        className={`mt-3 inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${
          programEquipment(program) === 'chair' ? 'bg-amber-500/15 text-amber-400' : 'bg-emerald-500/15 text-emerald-400'
        }`}
      >
        ...
        {program.days.map((day, dayIndex) => (
          <div key={day.name} className="rounded-2xl border border-slate-800 bg-slate-800/40 p-4">
            ...
            <p className="mb-3 flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
              {totalSets(day)} séries au total
              <span className="text-slate-700">·</span>
              <Clock size={12} />≈ {estimateDayMinutes(day)} min
              <span className="text-slate-700">·</span>
              <span className={dayEquipment(day) === 'chair' ? 'text-amber-400' : 'text-emerald-400'}>
                {equipmentBadge[dayEquipment(day)]}
              </span>
            </p>
                ...
                return (
                  <li key={i} className="flex items-center justify-between text-sm">
                    <span className="flex items-center gap-1.5 text-slate-300">
                      {exercise?.equipment === 'chair' && <span title="Nécessite une chaise">🪑</span>}
